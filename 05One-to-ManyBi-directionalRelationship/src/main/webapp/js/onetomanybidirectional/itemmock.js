...
function create(){
	...
	$.ajax({url : "/onetomanybidirectional/mock/create",type: "POST",
...
function loadObjects(){
	$.ajax({
		url : "/onetomanybidirectional/mock/findAll",
		type: "GET",
		data : {},
		dataType: "json",
		success: function(data, textStatus, jqXHR)
...
function addMoreFeature(){
	...
	itemeDiv.setAttribute("id","itemBox"+"_"+counter);
	...
	itemeDiv.innerHTML = "<label>Feature:"+"<input type='text' id='feature' name='feature' maxlength='12' size='10'/>" +
	"</label>"+removeLinkUrl;
...
function deleteObject(itemid){
	...
	delurl="/onetomanybidirectional/mock/remove/"+itemid;
	$.ajax({
		url : delurl,
		type: "POST",
		data : itemForm,
...
function update(){
	...
	$.ajax({
		url : "/onetomanybidirectional/mock/edit",
		type: "POST",
...
function editObject(itemid){
	editurl="/onetomanybidirectional/mock/findById/"+itemid;
	var itemForm={id:itemid};
