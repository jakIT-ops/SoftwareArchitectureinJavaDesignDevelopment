...
function isPresent(){
	...
	var workername1 = $("#workerSelectBox1").find('option:selected').text();
	var workername2 = $("#workerSelectBox2").find('option:selected').text();
	...
	if(null != workerid1 && "" != workerid1 && null!= workerid2 && "" != workerid2 && null!= relationshipType && "" != relationshipType) {
		...
		$.ajax({
			url : "/manytomanyselfreferencewithjoinattribute/workerworker/mock/isPresent",
			type: "POST",
			...
			{
				if(!data) {
					createWorkerWorker(workerid1, workerid2,relationshipType,workername1,workername2);
				} else {
...
function createWorkerWorker(workerid1,workerid2,relationshipType,workername1,workername2){
	var formData={"workerId1":{"id":workerid1, "name":workername1},"workerId2":{"id":workerid2, "name":workername2},"relationshipType":relationshipType};
	if(workerid1 != workerid2) {
		$.ajax({
			url : "/manytomanyselfreferencewithjoinattribute/workerworker/mock/create",
			type: "POST",
			data : JSON.stringify(formData),
			beforeSend: function(xhr) {
				xhr.setRequestHeader("Accept", "application/json");
...
function createWorker(){
	...
	if(null != name && "" != name) {
		...
		$.ajax({
			url : "/manytomanyselfreferencewithjoinattribute/worker/mock/create",
			type: "POST",
			data : JSON.stringify(formData),
			beforeSend: function(xhr) {
				xhr.setRequestHeader("Accept", "application/json");
...
function updateWorker(){
	...
	$.ajax({
		url : "/manytomanyselfreferencewithjoinattribute/worker/mock/edit",
		type: "POST",
		data : JSON.stringify(formData),
		beforeSend: function(xhr) {
			xhr.setRequestHeader("Accept", "application/json");
...
function loadObjects(){
	$.ajax({
		url : "/manytomanyselfreferencewithjoinattribute/worker/mock/findAll",
		type: "GET",
		data : {},
		dataType: "json",
		success: function(data, textStatus, jqXHR)
	...
	$.ajax({
		url : "/manytomanyselfreferencewithjoinattribute/workerworker/mock/findAll",
		type: "GET",
		data : {},
		dataType: "json",
		success: function(data, textStatus, jqXHR)
...
function deleteWorker(workerid){
	...
	delurl="/manytomanyselfreferencewithjoinattribute/worker/mock/remove/"+workerid;
	$.ajax({
		url : delurl,
		type: "POST",
		data : workerForm,
...
function deleteWorkerWorker(workerid1,workerid2){
	var formData={"workerId1":{"id":workerid1},"workerId2":{"id":workerid2}};
	delurl="/manytomanyselfreferencewithjoinattribute/workerworker/mock/remove";
	$.ajax({
		url : delurl,
		type: "POST",
		data : JSON.stringify(formData),
...
function editWorker(workerid){
	editurl="/manytomanyselfreferencewithjoinattribute/worker/mock/findById/"+workerid;
	var workerForm={id:workerid};
	$.ajax({
		url : editurl,
		type: "GET",
...
function processWorker2ResponseData(responsedata){
	var dyanamicTableRow="<table border=1>"+
	"<tr>" +
	"<td>Worker Name</td>"+"<td>Actions</td>"+
	"</tr>";
