...
function create(){
	...
	if(phone.length > 0){numbers =  new Array();for(var i=0;i<phone.length;i++)
	{
		if(trim(phone[i].value)!=""){
			numbers.push(phone[i].value);flag=true;
		}
	}
	}
	if(flag==false){numbers=null;}
	var formData={"name":name,"numbers":numbers};
	$.ajax({url : "/onetomanyunidirectional/create",type: "POST",
...
function loadObjects(){
	$.ajax({
		url : "/onetomanyunidirectional/findAll",type: "GET",data : {},dataType: "json",
...
function generateTableData(object){
	...
	if(phones!=null && phones.length > 0){for(var index=0;index<phones.length;index++)
	{ var phone = phones[index];dataRow = dataRow+"<option value="+phone+">"+phone+"</option>";}}
...
function addMorePhone(){
	...
	phoneDiv.innerHTML = "<label>Phone:"+"<input type='text' id='phone' name='phone' mqxlength='12' size='10'/>" +
	"</label>"+removeLinkUrl;
...
function deleteObject(personid){
	var customerForm={id:personid};
	delurl="/onetomanyunidirectional/remove/"+personid;
	$.ajax({url : delurl,type: "POST",data : customerForm,dataType: "json",
...
function update(){
	...
	var flag= false;
	var phone=document.getElementsByName("phone");
	if(phone.length > 0){numbers =  new Array();for(var i=0;i<phone.length;i++){if(trim(phone[i].value)!=""){numbers.push(phone[i].value);flag=true;}}}
	if(flag==false){numbers=null;}
	var formData={"id":id,"name":name,"numbers":numbers};
	$.ajax({
		url : "/onetomanyunidirectional/edit",
		type: "POST",
...
function editObject(personid){
	editurl="/onetomanyunidirectional/findById/"+personid;
	var customerForm={id:personid};
	$.ajax({url : editurl,type: "GET",data : customerForm,dataType: "json",
		success: function(data, textStatus, jqXHR){
			viewObject(data);
